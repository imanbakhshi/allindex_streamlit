...
# with st.sidebar:
#     start_date = st.date_input("تاریخ شروع", value=datetime(2024, 1, 1), min_value=datetime(2000, 1, 1),
#                                max_value=datetime.today())
#     end_date = st.date_input("تاریخ پایان", value=datetime(2024, 2, 15), min_value=datetime(2000, 1, 1),
#                              max_value=datetime.today())
...
#             }).rename("GCVI")
#
#             composite = ndvi.addBands([savi, mndwi, gcvi]).clip(region)
#
...
#             Map.addLayer(mndwi, {'min': -1, 'max': 1, 'palette': ['blue', 'white', 'red']}, "MNDWI", False)
...
#                     geemap.ee_export_image(image, filename=temp_path, scale=scale, region=region.geometry())
